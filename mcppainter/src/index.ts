...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (request.params.name === "use_eraser") {
    const { width } = request.params.arguments as { width: number };
    ...
    if (!response.ok) throw new McpError(ErrorCode.InternalError, "橡皮擦设置失败");
    return { content: [{ type: "text", text: "橡皮擦设置成功" }] };
  }
  if (request.params.name === "set_brush") {
    const { width, color } = request.params.arguments as { width: number; color: string };
    ...
    if (!response.ok) throw new McpError(ErrorCode.InternalError, "画笔设置失败");
    return { content: [{ type: "text", text: "画笔设置成功" }] };
  }
  if (request.params.name === "set_color") {
    const { color } = request.params.arguments as { color: string };
    ...
    if (!response.ok) throw new McpError(ErrorCode.InternalError, "颜色设置失败");
  ...
  if (request.params.name === "get_canvas_size") {
    ...
    if (!response.ok) throw new McpError(ErrorCode.InternalError, "获取画布大小失败");
