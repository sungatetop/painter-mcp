...
def draw_point(x, y, is_start=False, is_end=False):
    """绘制点"""
    url = f'{BASE_URL}/draw'
    data = {'x': x, 'y': y, 'isStart': is_start, 'isEnd': is_end}
    response = requests.post(url, json=data)
    if response.status_code != 200:
...
def draw_star(center_x, center_y, radius):
    ...
    for i in range(5):
        # 开始绘制
        draw_point(points[i][0], points[i][1], is_start=True)
        # 连接到下一个点
        next_point = points[(i + 2) % 5]
        draw_point(next_point[0], next_point[1])
    
    # 结束绘制
    draw_point(0, 0, is_end=True)
